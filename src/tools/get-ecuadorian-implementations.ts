/**
 * get_ecuadorian_implementations — Find Ecuadorian statutes implementing a specific EU directive/regulation
...
export interface GetEcuadorianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface EcuadorianImplementationResult {
  document_id: string;
...
export async function getEcuadorianImplementations(
  db: InstanceType<typeof Database>,
  input: GetEcuadorianImplementationsInput,
): Promise<ToolResponse<EcuadorianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as EcuadorianImplementationResult[];
