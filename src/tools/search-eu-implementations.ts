/**
 * search_eu_implementations — Search EU directives/regulations that have Ecuadorian implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_ecuadorian_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  ecuadorian_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as ecuadorian_statute_count
  ...
  if (input.has_ecuadorian_implementation) {
    sql += ' HAVING ecuadorian_statute_count > 0';
  }
