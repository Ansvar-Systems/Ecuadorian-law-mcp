#!/usr/bin/env tsx
/**
 * Drift detection for Ecuador Law MCP.
 *
 * Checks if upstream Ecuador Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Ecuador Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'ecuadorian-law-mcp/1.0 drift-detect' },
      });
