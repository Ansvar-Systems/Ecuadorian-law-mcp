#!/usr/bin/env tsx
/**
 * Database builder for Ecuador Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Ecuador Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Ecuadorian statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'EC');
    insertMeta.run('source', 'registroficial.gob.ec');
