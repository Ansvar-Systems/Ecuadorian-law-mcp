#!/usr/bin/env tsx
/**
 * Ecuadorian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://registroficial.gob.ec';
const PORTAL_NAME = 'Ecuador Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/ecuadorian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Ecuadorian Law MCP — Data Freshness Check');
