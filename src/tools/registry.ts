/**
 * Tool registry for Ecuador Law MCP Server.
...
import { getEcuadorianImplementations, type GetEcuadorianImplementationsInput } from './get-ecuadorian-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including Ecuador Law (National Council for Law Reporting). ' +
    ...
    'Call this FIRST when you need to understand what Ecuadorian legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Ecuadorian statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section/article) from a Ecuadorian statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate a Ecuadorian legal citation against the database — zero-hallucination check. ' +
      ...
      'Supports formats: "Section 25, Data Protection Act 2019", "s 25, DPA 2019", "Article 31, Constitution of Ecuador 2010".',
    inputSchema: {
      type: 'object',
      properties: {
        citation: {
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Ecuadorian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Ecuador?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Ecuadorian legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether a Ecuadorian statute or provision is currently in force, amended, repealed, partially suspended, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU/international legal basis that a Ecuadorian statute aligns with or references. ' +
      'Ecuador is not an EU member but several Ecuadorian laws were influenced by EU regulations ' +
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Ecuadorian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_ecuadorian_implementations',
    description:
      'Find all Ecuadorian statutes that align with or implement a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Ecuadorian statutes. ' +
      'Note: Ecuador implements international standards through domestic legislation influenced by international frameworks.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that have Ecuadorian implementing/aligning legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_ecuadorian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents with Ecuadorian aligning legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU/international legal basis for a SPECIFIC provision within a Ecuadorian statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU/international alignment status for a Ecuadorian statute or provision. ' +
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_ecuadorian_implementations':
          result = await getEcuadorianImplementations(db, args as unknown as GetEcuadorianImplementationsInput);
          break;
